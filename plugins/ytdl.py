...
async def fast_upload(client, file_path, reply=None, name=None, progress_bar_function=None):
    """Upload a file with Telethon without external branded helpers."""
    def _progress(done, total):
        if not progress_bar_function:
            return
        try:
            result = progress_bar_function(done, total)
            if asyncio.iscoroutine(result):
                asyncio.get_event_loop().create_task(result)
        except Exception as exc:
            logger.warning(f"Upload progress update failed: {exc}")

    return await client.upload_file(
        file_path,
        file_name=name or os.path.basename(file_path),
        progress_callback=_progress if progress_bar_function else None
    )
...
async def process_audio(client, event, url, cookies_env_var=None):
    ...
    random_filename = f"audio_{event.sender_id}_{get_random_string()}"
    ...
    try:
        ...
        if os.path.exists(download_path):
            def edit_metadata():
                ...
                audio_file.tags["TPE1"] = TPE1(encoding=3, text="Telegram Downloader")
                audio_file.tags["COMM"] = COMM(encoding=3, lang="eng", desc="Comment", text="Processed by Telegram Downloader")
