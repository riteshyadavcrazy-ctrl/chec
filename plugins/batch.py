...
async def upd_dlg(c, limit=1000):
    """Refresh Pyrogram's local peer cache.

    Pyrogram can throw CHAT_ID_INVALID for channels/groups that the account has
    joined but has not loaded into its peer cache yet. Walking dialogs forces the
    client to learn those peers before get_messages/get_chat_member is called.
    """
    if not c:
        return False
    try:
        async for _ in c.get_dialogs(limit=limit):
            pass
        return True
    except Exception as e:
...
def _chat_candidates(chat_ref):
    """Build common Pyrogram chat-id candidates for public/private links."""
    ...
    if raw.startswith('-100') and raw[4:].isdigit():
        base = raw[4:]
        candidates.extend([int(raw), f'-{base}', int(f'-{base}')])
    elif raw.startswith('-') and raw[1:].isdigit():
        base = raw[1:]
        candidates.extend([int(raw), f'-100{base}', int(f'-100{base}')])
    elif raw.isdigit():
        candidates.extend([f'-100{raw}', int(f'-100{raw}'), f'-{raw}', int(f'-{raw}'), int(raw)])

    return _unique(candidates)


async def _known_dialog_chat_id(client, chat_ref, limit=2000):
    """Find the chat in loaded dialogs and return the exact Pyrogram chat id."""
    if not client:
        return None

    raw = str(chat_ref or '').strip().lstrip('@').lower()
    numeric_candidates = set()
    for candidate in _chat_candidates(chat_ref):
        try:
            numeric_candidates.add(int(candidate))
        except Exception:
            pass
    ...
    try:
        async for dialog in client.get_dialogs(limit=limit):
            ...
            if chat_id in numeric_candidates:
                return chat_id

            username = (getattr(chat, 'username', None) or '').lower()
            if raw and username == raw:
                return chat_id
    except Exception as e:
        print(f'Known dialog scan failed: {e}')

    return None


async def _get_message_safely(client, chat_ref, message_id, allow_join=False, refresh=True):
    """Resolve a chat then fetch one message without failing on stale peer cache."""
    if not client:
        return None
    ...
    for candidate in _chat_candidates(chat_ref):
        try:
            msg = await client.get_messages(candidate, message_id)
            if msg and not getattr(msg, 'empty', False):
                return msg
        except Exception as e:
            last_error = e

    # Public channels/groups may need an explicit get_chat/join_chat first.
    for candidate in _chat_candidates(chat_ref):
        ...
        if text.lstrip('-').isdigit():
            continue
        try:
            lookup = text if text.startswith('@') else f'@{text}'
            if allow_join:
                try:
                    await client.join_chat(lookup)
                except Exception:
                    pass
    ...
    if refresh:
        await upd_dlg(client, limit=2000)
        known_id = await _known_dialog_chat_id(client, chat_ref, limit=2000)
        if known_id is not None:
            try:
                msg = await client.get_messages(known_id, message_id)
                if msg and not getattr(msg, 'empty', False):
                    return msg
            except Exception as e:
                last_error = e
    ...
    if last_error:
        print(f'Could not fetch message {message_id} from {chat_ref}: {last_error}')
    return None


# Compatibility handler for older group link formats
async def get_msg(c, u, i, d, lt):
    try:
        if lt == 'public':
            # First try the user's custom bot. If the bot has not seen the chat,
            # fall back to the logged-in user client and let it join/resolve public chats.
            msg = await _get_message_safely(c, i, d, allow_join=False, refresh=True)
            if msg:
                emp[i] = False
                print(f'fetched by {getattr(getattr(c, "me", None), "username", "bot")}')
                return msg

            msg = await _get_message_safely(u, i, d, allow_join=True, refresh=True)
            if msg:
                emp[i] = True
                print('fetched by user client')
                return msg

            emp[i] = True
            return None

        # Private /c/ links need a user session that is a member of that chat.
        if not u:
            return None

        await upd_dlg(u, limit=2000)
        msg = await _get_message_safely(u, i, d, allow_join=False, refresh=True)
        if msg:
            return msg

        print(
            f'Private chat {i} is not known to the user client. '
            'Make sure the logged-in user account has joined/opened that channel/group.'
        )
        return None
...
async def process_cmd(c, m):
    ...
    if FREEMIUM_LIMIT == 0 and not await is_premium_user(uid):
        await m.reply_text("This bot does not provide free service. Premium access is required.")
        return
